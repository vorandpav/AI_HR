...
from backend.services.stt_tts_client import get_stt_client, STTClient
from backend.routers.meetings import get_meeting_by_token, finish_meeting_sync

logger = logging.getLogger("uvicorn.error")
router = APIRouter()


async def client_to_stt(client_ws: WebSocket, stt_ws: aiohttp.ClientWebSocketResponse, session_id: str):
    ...
    try:
        while True:
            ...
            if data:
                ...
                try:
                    await anyio.to_thread.run_sync(save_audio_chunk_sync, data, session_id, "participant", "audio/webm")
                    logger.debug(f"Saved client audio chunk for session {session_id}")
                except Exception as e:
                    logger.exception("Failed saving client audio chunk for session %s", session_id)
...
async def stt_to_client(stt_ws: aiohttp.ClientWebSocketResponse, client_ws: WebSocket, session_id: str):
    ...
    try:
        async for msg in stt_ws:
            if msg.type == aiohttp.WSMsgType.BINARY:
                ...
                try:
                    await anyio.to_thread.run_sync(save_audio_chunk_sync, msg.data, session_id, "bot", "audio/webm")
                    logger.debug(f"Saved bot audio chunk for session {session_id}")
                except Exception as e:
                    logger.exception("Failed saving bot audio chunk for session %s", session_id)
...
async def call_ws(websocket: WebSocket, token: str):
    ...
    logger.info("New call session %s for meeting %d (token=%s)", session_id, meeting.id, token)
    ...
    try:
        ...
        logger.info("Successfully connected to STT/TTS service for session %s", session_id)

        # Запускаем задачи обмена аудио
        task_client_to_stt = asyncio.create_task(client_to_stt(websocket, stt_ws, session_id))
        task_stt_to_client = asyncio.create_task(stt_to_client(stt_ws, websocket, session_id))
        ...
        logger.info(f"Call session {session_id} tasks completed. Done: {len(done)}, Pending: {len(pending)}")
        ...
        logger.exception("Failed to establish or maintain connection for session %s: %s", session_id, e)
        ...
        try:
            ...
            logger.exception("Failed to finish meeting for token %s in DB: %s", token, e)
        ...
        try:
            ...
            asyncio.create_task(post_processing.process_and_merge_audio(meeting.id, session_id))
            logger.info(f"Post-processing task scheduled for meeting {meeting.id}, session {session_id}")
        except Exception as e:
            logger.exception(f"Failed to schedule post-processing for meeting {meeting.id}: {e}")
