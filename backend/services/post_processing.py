...
from .minio_client import get_minio_client, DEFAULT_BUCKET
...
def _cleanup_source_data(object_keys: List[str], object_ids: List[int]):
    ...
    try:
        if object_ids:
            db.query(models.AudioObject).filter(models.AudioObject.id.in_(object_ids)).delete(synchronize_session=False)
            db.commit()
            logger.info(f"Deleted {len(object_ids)} source AudioObject records from DB.")
    except Exception as e:
...
async def _download_audio_object_bytes(object_key: str) -> bytes:
    ...
    def _sync_get():
        ...
        response = minio_client.client.get_object(
            bucket_name=DEFAULT_BUCKET,  # ИСПРАВЛЕНО
...
def _concatenate_audio_files(input_files: List[str], output_file: str):
    ...
    logger.info(f"Starting FFmpeg concatenation of {len(input_files)} files into {output_file}")
    try:
        # Если файл один, просто копируем его, чтобы не вызывать сложную логику FFmpeg.
        if len(input_files) == 1:
            ffmpeg.input(input_files[0]).output(output_file, acodec='copy').overwrite_output().run(quiet=True)
        ...
        (
            ffmpeg
            .output(concatenated_stream, output_file, acodec='copy')
            .overwrite_output()
            .run(quiet=True)
        )
...
def _mix_audio_tracks_ffmpeg(track_files: List[str], output_file: str):
    ...
    if len(track_files) == 1:
        ...
        ffmpeg.input(track_files[0]).output(output_file, acodec='libopus', audio_bitrate='128k').overwrite_output().run(
            quiet=True)
        return
    ...
    try:
        ...
        mixed_audio = ffmpeg.filter(inputs, 'amix', inputs=len(inputs), duration='longest')

        (
            ffmpeg
            .output(mixed_audio, output_file, acodec='libopus', audio_bitrate='128k')
            .overwrite_output()
            .run(quiet=True)
        )
        logger.info(f"Successfully mixed tracks into {output_file}")
    except ffmpeg.Error as e:
        logger.error(f"FFmpeg error during mixing: {e.stderr.decode('utf8') if e.stderr else 'No stderr'}")
        raise
...
def _save_final_file_and_update_db(meeting_id: int, session_id: str, file_path: str):
    ...
    try:
        ...
        db.add(ao)
        db.commit()
        logger.info(f"Final AudioObject record created in DB for key {final_object_name}")
    except Exception as e:
...
async def process_and_merge_audio(meeting_id: int, session_id: str):
    ...
    """
    logger.info(f"Starting post-processing for meeting {meeting_id}, session {session_id}")
    ...
    try:
        ...
        audio_objects = db.query(models.AudioObject).filter(
            models.AudioObject.session_id == session_id,
            models.AudioObject.is_final == False
        ).order_by(models.AudioObject.created_at).all()

        if not audio_objects:
            logger.warning(f"No audio chunks found for session {session_id}. Aborting post-processing.")
            return
        ...
        chunks_by_role: Dict[str, List[models.AudioObject]] = {"participant": [], "bot": []}
        ...
        for role, chunks in chunks_by_role.items():
            ...
            download_tasks = [_download_audio_object_bytes(chunk.object_key) for chunk in chunks]
            ...
            if any(chunks_data):
                with tempfile.NamedTemporaryFile(delete=False, suffix=f'_{role}_track.webm') as track_file:
                    for data in chunks_data:
                        if data:
                            track_file.write(data)

                    track_path = track_file.name
                    logger.info(f"Created single concatenated track for role '{role}' at {track_path}")
        ...
        with tempfile.NamedTemporaryFile(delete=False, suffix='_final_mixed.ogg') as final_output_file:
        ...
        await loop.run_in_executor(
            None, _save_final_file_and_update_db, meeting_id, session_id, final_output_path
        )
        logger.info(f"Successfully mixed and saved final recording for session {session_id}")

        # 6. Очистка исходных данных (чанки в MinIO и записи в БД)
        await loop.run_in_executor(None, _cleanup_source_data, source_object_keys, source_object_ids)

    except Exception as e:
        logger.exception(f"Critical error during post-processing for meeting {meeting_id}: {e}")
    finally:
        # 7. Гарантированная очистка временных файлов на диске
        _cleanup_temp_files(all_temp_files)
        db.close()
        logger.info(f"Post-processing finished for meeting {meeting_id}, session {session_id}")
