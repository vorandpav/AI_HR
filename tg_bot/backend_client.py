...
class BackendClient:
    ...
    async def post_vacancy(self,
        ...
        if file_bytes is not None:
            form.add_field("file", file_bytes, filename=filename or "file",
                           content_type=mime or "application/octet-stream")
        async with aiohttp.ClientSession() as session:
            async with session.post(f"{self.base}/vacancies/", data=form,
    ...
    async def post_resume(self,
                          vacancy_id: int,
                          telegram_username: str,
                          telegram_user_id: str,
                          file_bytes: bytes,
                          filename: str, mime: str, timeout=30):
        form = aiohttp.FormData()
        form.add_field("vacancy_id", str(vacancy_id))
        form.add_field("telegram_username", telegram_username)
        form.add_field("telegram_user_id", telegram_user_id)
        form.add_field("file", file_bytes, filename=filename, content_type=mime or "application/octet-stream")
        async with aiohttp.ClientSession() as session:
            async with session.post(f"{self.base}/resumes/", data=form,
                                    timeout=aiohttp.ClientTimeout(total=timeout)) as resp:
                resp.raise_for_status()
                return await resp.json()
    ...
    async def get_similarity(self, resume_id: int, x_telegram_user: str):
        headers = {"X-Telegram-User": x_telegram_user} if x_telegram_user else {}
        async with aiohttp.ClientSession(headers=headers) as session:
            async with session.get(f"{self.base}/similarity/resume/{resume_id}") as resp:
                if resp.status == 200:
                    return await resp.json()
    ...
    async def arrange_meeting(self, resume_id: int, organizer_username: str):
        ...
        async with aiohttp.ClientSession(headers=headers) as session:
            async with session.post(f"{self.base}/arrange_meeting", json={"resume_id": resume_id},
                                    timeout=aiohttp.ClientTimeout(total=10)) as resp:
                resp.raise_for_status()
                return await resp.json()

    async def download_recording_by_resume_id(self, resume_id: int, x_telegram_user: str) -> Tuple[bytes, str, str]:
        """
        ...
        headers = {"X-Telegram-User": x_telegram_user}
        async with aiohttp.ClientSession(headers=headers) as session:
            async with session.get(f"{self.base}/resumes/{resume_id}/recording") as resp:
